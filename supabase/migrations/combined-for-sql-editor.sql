-- ════════════════════════════════════════════════════════════════════
-- PackShot — Complete Database Schema
-- Run this ONCE in Supabase SQL Editor
-- Project: eqpnvxfccemmbulhczdb
-- ════════════════════════════════════════════════════════════════════
...
CREATE POLICY "Users can view own ai keys" ON public.user_ai_keys FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can manage own ai keys" ON public.user_ai_keys FOR ALL USING (auth.uid() = user_id);
...
CREATE POLICY "Service role full access usage" ON public.usage FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Service role full access api_keys" ON public.api_keys FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY "Service role full access user_ai_keys" ON public.user_ai_keys FOR ALL USING (auth.role() = 'service_role');
...
-- ─── MIGRATION 002: Webhooks (Studio API) ───────────────────────────
...
CREATE POLICY "Service role full access webhooks" ON public.webhooks FOR ALL USING (auth.role() = 'service_role');
...
CREATE POLICY "Service role full access email_queue" ON public.email_queue FOR ALL USING (auth.role() = 'service_role');
